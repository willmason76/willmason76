...
# This notebook, based on several online tutorials, is meant to be a basic example of the Clarke Wright savings algorithm, as applied for deliveries from a single hub to locations in the Las Vegas area. Please note I did not overlay the road system with Google API, but merely assumes direct travel from hub to nodes and back for simplicity. 
# 
...
# 
# Here are the graphed cost functions for each truck  __y = mx^(1-n)__ :
# 
...
# 
# The excel file is included on the FedEx GitHub page.
# 
...
print('The number of individual deliveries based on the weight distribution and truck capacity is:',len(df_new))


# ## Starting Location
# 
# Just to take another look at hub and delivery locations...
...
plt.scatter(df_locations['lon'],df_locations['lat'] ,marker = 'o', s = 35, c = 'blue')
plt.scatter(source_coordinates[1],source_coordinates[0], marker = '*', s = 250, c = 'black')
...
planning_horizon = 1      #Number of days between deliveries
...
df_final = pd.DataFrame(result_matrix, columns=['Date', 'Cost with Routing', 'Cost without Routing'])
...
cost_no_vr = sum(df_final['Cost without Routing'])
cost_vr = sum(df_final['Cost with Routing'])
...
df_final['Cost Differential'] = df_final['Cost without Routing'] - df_final['Cost with Routing'] 
...
# 
# The cost differential, when the planning horizon is set to one day, isn't always positive, although does appear to change to universal cost savings when planning horizon > 1. But the cumulative pattern over any horizon demonstrates that using the Clarke Wright approach is more efficient over time.
# 
# The synthetic data I produced seems to invert the cost functions, might be missing something obvious here.
# 
# ## Conclusion
# 
